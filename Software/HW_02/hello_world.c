...
int main()
{
	...
	mailbox_sender = altera_avalon_mailbox_open("/dev/mailbox_simple_2", tx_cb,
	NULL);
	mailbox_rcv = altera_avalon_mailbox_open("/dev/mailbox_simple_3", NULL,
	rx_cb);
	if (!mailbox_rcv){
...
//		  alt_u32 display_end_t = alt_timestamp();
	      lossless_decode(ld_message->num_blocks, ld_message->bitstream, ld_message->DCACq, Cquant,
	     ...
	     // for (int h = hCb_size; h < 4*hCb_size/5; h++)
	       for (int h = 0; h < (5*hCb_size/10)+3; h++)
