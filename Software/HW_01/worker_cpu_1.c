/*
 * Worker CPU 1 for the FPGA MJPEG decoder.
 *
 * This core receives commands from the main decoder over mailbox_simple_0,
 * sends acknowledgements over mailbox_simple_1, decodes the Y lossless stream,
 * and converts the middle slice of the frame to the display buffer.
 */
...
#define MAILBOX_TX_DEVICE "/dev/mailbox_simple_1"
#define MAILBOX_RX_DEVICE "/dev/mailbox_simple_0"
        ...
        altera_avalon_mailbox_dev **receiver)
{
    ...
    *receiver = altera_avalon_mailbox_open(MAILBOX_RX_DEVICE, NULL, rx_cb);
    ...
    printf("PASS: Opened worker CPU 1 mailboxes\n");
    return 0;
}
...
static void handle_lossless_decode(alt_u32 *message)
{
    ...
    lossless_decode(ld_message->num_blocks, ld_message->bitstream,
            ld_message->DCACq, Yquant, ld_message->frame_type);
...
static void handle_color_convert(alt_u32 *message)
{
    ...
    int first_row = (5 * hCb_size / 10) + 3;
    int last_row = 8 * hCb_size / 10;
...
static void rx_cb(void *message)
{
    ...
    if (message != NULL) {
        printf("Message received from core 1 %d, %d", data[0], data[1]);
    } else {
